...
# tests/test_graperank.py to cross-validate a production-compatible algorithm
# replica. If both pulled from shared/, a silent bug in the shared values would
# pass the cross-check. Keep the numbers in lockstep with
# NosFabrica/brainstorm_graperank_algorithm Constants.java — there's a test
# (test_graperank.py::test_constants_match_shared) that asserts parity.
